...
{
    [LabelText("房间参数")]
    public AreaParamResources AreaParamResources;

    [SerializeField]
    [Range(1, 500)]
    [LabelText("房间位置随机限制系数R（房间位置满足：位置+R半径内球形随机")]
    int Radius = 50;

    [SerializeField]
    [Range(0, 2.0f)]
    [LabelText("主要区域限制系数M（主要区域满足：区域尺寸 > widthAvg*M）")]
    float MainRoomFrequency = 1f;

    [SerializeField]
    [Range(0, 1)]
    [LabelText("蓝色次级道路限制系数C（蓝色连线满足：C值越小蓝色道路出现越少）")]
    ...
    void Init()
    {
        ...
        if (DungeonMapTexture != null)
            DungeonMapTexture.GetComponent<MeshRenderer>().material.mainTexture = null;
        else
        ...
        else
        {
            ...
            RoomGenerator.DG = this;
        }

        //RoomGenerator.OnRoomsGenerated += RoomGenerator_OnRoomsGenerated;

        //Generates rooms and room connections
       RoomGenerator.Generate(AreaParamResources, Radius, MainRoomFrequency, RoomConnectionFrequency);
    }
    ...
    /*
    public void RoomGenerator_OnRoomsGenerated()
    {

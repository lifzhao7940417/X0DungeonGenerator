...
public class Room : MonoBehaviour
{
    ...
    public void Init(int id, Vector2 position, int width, int height, AreaDes inDes)
    {
        ID = id;
        Des = inDes;
        Background.color = inDes.Color;
        transform.position = new Vector2(position.x, position.y);
        transform.localScale = new Vector2(width, height);

        this.transform.GetChild(0).GetComponent<TextMesh>().text = inDes.Tag.ToString();
    }
